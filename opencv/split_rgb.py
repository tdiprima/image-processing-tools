"""
Provides functionalities for image processing such as load an image to extract, separate and manipulate RGB color channels,
convert the image to greyscale, invert a greyscale, display the image, save image or RGB data to a file and plot color histograms.
...
def jpg_image_to_array1(image_path):
    """
    Use numpy from-iter to invert a greyscale
    :param image_path:
    :return:
    """
...
def jpg_image_to_array2(image_path):
    """
    Use the to-bytes function of the Image object.
...
def get_channel():
    ...
    # save image
    cv.imwrite('cv2-green-channel.png', green_img)


def plot_histogram():
    # https://datatofish.com/plot-histogram-python/
    src = cv.imread('../images/prob.png')
    print(src.shape)
    green_channel = src[:, :, 1]
    print('Unique values of array:')
    u = np.unique(green_channel)
    print(u)
    plt.hist(u)
    plt.show()
